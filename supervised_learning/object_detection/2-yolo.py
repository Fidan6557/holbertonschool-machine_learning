...
class Yolo():
    ...
    def process_outputs(self, outputs, image_size):
        ...
        for ipred, pred in enumerate(boxes):
            for grid_h in range(pred.shape[0]):
                for grid_w in range(pred.shape[1]):
                    ...
                    anchor_tensor[:, 0] *= (
                        np.exp(pred[grid_h, grid_w, :, 2]) /
                        self.model.input.shape[1]
                    )
                    anchor_tensor[:, 1] *= (
                        np.exp(pred[grid_h, grid_w, :, 3]) /
        ...
        box_confidences = [
            self.sigmoid(pred[:, :, :, 4:5]) for pred in outputs
        ]
        box_class_probs = [
            self.sigmoid(pred[:, :, :, 5:]) for pred in outputs
        ]
    ...
    def filter_boxes(self, boxes, box_confidences, box_class_probs):
        ...
        for boxes_i, confidences_i, probs_i in zip(
